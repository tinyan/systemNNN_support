...
BOOL CPackRoutine::ArcDWQ(LPSTR dstDir,int version,LPSTR dstDir2)
{
	//
...
BOOL CPackRoutine::PackRoutine0(LPSTR srcDir,LPSTR filename,LPSTR extName,LPSTR dstDir)
{
	...
	//FILE* file2 = fopen(dstFileName,"wb");
	if (file2 == NULL)
	{
		PrintFatalError(filename,"SaveOpenError");
		return FALSE;
	}
...
//	}
	...
	char dstFileName[256];
	sprintf(dstFileName,"%s\\%s.dwq",dstDir,filename);
...
{
	char srcFileName[256];
	sprintf(srcFileName,"%s\\%s.jpg",srcDir,filename);
...
RECT CPackRoutine::CalcuCutData(LPSTR dirName,LPSTR filename)
{
	...
	int i,j;
	char srcFileName[256];
	sprintf(srcFileName,"%s\\%sm.bmp",dirName,filename);
