...
bool IsChangeXaudio2(void);
void GetXAudioCheck(void);
void SetVolumeAdjust(void);
void RecalcuVolume(void);
...
//char m_defaultCddaVolumeCaption[] = "‰¹Šy(CDDA)";
...
void SetUseWin10ExpButton(void);
...
void GetUseWin10ExpButton(void);
...
int m_volumeMin = 0;
int m_volumeMax = 100;
int m_volumeMinDirectSound = 0;
int m_volumeMaxDirectSound = 100;
int m_volumeMinXAudio2 = 0;
int m_volumeMaxXAudio2 = 100;
...
//HWND m_useXAudio2Button = NULL;
//HWND m_useDirect2DButton = NULL;

HWND m_useXAudio2RadioAuto = NULL;
HWND m_useXAudio2RadioOff = NULL;
HWND m_useDirect2DRadioAuto = NULL;
HWND m_useDirect2DRadioOff = NULL;
...
//	LoadSetupFile2("mainControl",512);
	...
	if (volumeAdjustFlag)
	{
		GetInitGameParam3(&m_volumeMinDirectSound, "volumeMin");
		GetInitGameParam3(&m_volumeMaxDirectSound, "volumeMax");

		m_volumeMin = m_volumeMinDirectSound;
		m_volumeMax = m_volumeMaxDirectSound;

		m_volumeMinXAudio2 = m_volumeMin;
		m_volumeMaxXAudio2 = m_volumeMax;

		GetInitGameParam3(&m_volumeMinXAudio2,"volumeMinXAudio2");
		GetInitGameParam3(&m_volumeMaxXAudio2,"volumeMaxXAudio2");
...
//		m_volumeMin = m_volumeMinXAudio2;
//		m_volumeMax = m_volumeMaxXAudio2;
	}
...
//	mx = CreateMutex(NULL,TRUE,"BaraByTinyan");
	...
	if (GetLastError() == ERROR_ALREADY_EXISTS)
	{
		if (m_codeByte != 1)
		{
			MessageBox(NULL,"“ñd‹N“®‹y‚ÑAƒQ[ƒ€“®ì’†‚Í‹N“®‚Å‚«‚Ü‚¹‚ñ","Error",MB_OK);
		}
		else
		{
...
//		ENDDELETECLASS(m_setup2);
	...
	if (LoadSystemFile() == FALSE)
	{
		...
		if (m_codeByte != 1)
		{
			wsprintf(mes,"%s\\%s.sav‚ª‚Ý‚Â‚©‚è‚Ü‚¹‚ñ‚Å‚µ‚½",saveFolder,m_systemFileName);
		}
		else
		{
...
BOOL CALLBACK DlgProc0( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_INITDIALOG:
		...
		if (m_useExpCheck > 0)
		{
			...
			tcItem[2].pszText = "Šg’£ƒ`ƒFƒbƒNÝ’è";
		...
		if (m_useExpRadio > 0)
		{
			...
			if (noCreate == 0)
			{
				...
				tcItem[3].pszText = "Šg’£ƒ‰ƒWƒIÝ’è";
				if (m_codeByte == 1)
				{
		...
		if (m_useExpRadio > 12)
		{
			...
			tcItem[4].pszText = "Šg’£ƒ‰ƒWƒIÝ’è2";
			if (m_codeByte == 1)
			{
		...
		if (m_screenSizeTypeMax > 1)
		{
			...
			tcItem[5].pszText = "ƒXƒNƒŠ[ƒ“ƒTƒCƒYÝ’è";
			if (m_codeByte == 1)
			{
...
//		m_useXAudio2Button = GetDlgItem(hWnd, IDC_CHECK_XAUDIO2);
//		m_useDirect2DButton = GetDlgItem(hWnd, IDC_CHECK_DIRECT2D);

		m_useXAudio2RadioAuto = GetDlgItem(hWnd, IDC_RADIO_XAUDIO2_AUTO);
		m_useXAudio2RadioOff = GetDlgItem(hWnd, IDC_RADIO_XAUDIO2_OFF);
...
void DataForSave(void)
{
	if (m_useDirect2DFlag) m_directDrawNotUse = 1;
	...
	if (m_useDirect2DFlag)
	{
		m_systemdata->genericFlag |= 4;
	}
	else
	{
		m_systemdata->genericFlag &= ~4;
...
void DataByLoad(void)
{
	...
	m_useXAudio2Flag = m_systemdata->genericFlag & 2;
	m_useDirect2DFlag = m_systemdata->genericFlag & 4;
...
void SetUseWin10ExpButton(void)
{
	if (m_useXAudio2Flag == 0)
	{
	//	SendMessage(m_useXAudio2Button, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(m_useXAudio2RadioAuto, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(m_useXAudio2RadioOff, BM_SETCHECK, BST_CHECKED, 0);
	}
	else
	{
//		SendMessage(m_useXAudio2Button, BM_SETCHECK, BST_CHECKED, 0);
		SendMessage(m_useXAudio2RadioAuto, BM_SETCHECK, BST_CHECKED, 0);
		SendMessage(m_useXAudio2RadioOff, BM_SETCHECK, BST_UNCHECKED, 0);
	}

	if (m_useDirect2DFlag == 0)
	{
//		SendMessage(m_useDirect2DButton, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(m_useDirect2DRadioAuto, BM_SETCHECK, BST_UNCHECKED, 0);
		SendMessage(m_useDirect2DRadioOff, BM_SETCHECK, BST_CHECKED, 0);
	}
	else
	{
		SendMessage(m_useDirect2DRadioAuto, BM_SETCHECK, BST_CHECKED, 0);
		SendMessage(m_useDirect2DRadioOff, BM_SETCHECK, BST_UNCHECKED, 0);
//		SendMessage(m_useDirect2DButton, BM_SETCHECK, BST_CHECKED, 0);
	}
...
void SetScreenStretchButton(void)
{
	...
	{
		SendMessage(m_screenStretchButton, BM_SETCHECK, BST_CHECKED, 0);
	}
}
...
void GetUseWin10ExpButton(void)
{
//	if (SendMessage(m_useXAudio2Button, BM_GETCHECK, 0, 0) == BST_CHECKED)
	if (SendMessage(m_useXAudio2RadioAuto, BM_GETCHECK, 0, 0) == BST_CHECKED)
	{
		m_useXAudio2Flag = 2;
	}
	else
	...
	//if (SendMessage(m_useDirect2DButton, BM_GETCHECK, 0, 0) == BST_CHECKED)
	if (SendMessage(m_useDirect2DRadioAuto, BM_GETCHECK, 0, 0) == BST_CHECKED)
	{
		m_useDirect2DFlag = 4;
	}
	else
